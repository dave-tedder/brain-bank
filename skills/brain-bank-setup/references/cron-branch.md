...
SKILL.md Reads this file when the operator selects option 1 on the post-Slack cron branch menu. Runs the guided walkthrough for `docs/deploy-from-scratch.md` Step 12 (vault secret + 4 cron schedules).
...
| **Vault create_secret** | "Run this in SQL editor. I'll give you the template. First argument is the raw secret (paste from clipboard), second is the name Postgres uses to look it up. Same value as .env." | Emit `select vault.create_secret('<PASTE FROM CLIPBOARD>', 'mcp_access_key', 'MCP / Brain access key, used by pg_cron to call Edge Functions.');` After operator confirms: `select name, description, created_at from vault.secrets where name = 'mcp_access_key';`, expect 1 row. | Run the SQL block, paste from clipboard into the marker. |
