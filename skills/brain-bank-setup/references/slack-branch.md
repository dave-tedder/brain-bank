...
| **5. Invite bot + grab channel IDs** | For each channel the operator created: "In `#channel-name`, type `/invite @Brain Bank`. Then right-click the channel in sidebar, **View channel details**, scroll to **Integrations** (or **Apps**) and confirm the bot is listed there (NOT under Members; modern Slack puts bots under Integrations). Then scroll to the bottom of the same panel, copy **Channel ID** (format: `C0123456789`). Paste into `.env` under the right variable name. I'll tell you which." Walk through one channel at a time: SLACK_CAPTURE_CHANNEL (required), then optional SLACK_BRAIN_CHANNEL, SLACK_QUERY_CHANNEL, SLACK_DIGEST_CHANNEL. | After each ID in `.env`, run `grep -cE '^SLACK_CAPTURE_CHANNEL=C[A-Z0-9]{10}$' .env` (and same per variable). Channel IDs aren't secrets; the grep can safely print the full line if needed for diagnostics. | Invite bot, confirm under Integrations, grab IDs, paste into `.env`. |
...
| **8. Event Subscriptions + URL verify + manual reinstall + token check** | "Back in Slack app settings, **Event Subscriptions**, toggle **Enable Events** on, paste this URL: `$SUPABASE_URL/functions/v1/ingest-thought` (no `?key=`, no trailing slash). Slack will verify within a second: look for green 'Verified'. Then scroll down to **Subscribe to bot events**, **Add Bot User Event**, `message.channels` (and `message.groups` if using private channels). Click Save Changes. Now go to **OAuth & Permissions**, scroll to top, click **Reinstall to Workspace** (or **Reinstall App**), Allow. Do this manually every time, even if Slack does not prompt; the prompt is unreliable. Then check the **Bot User OAuth Token** value at the top of OAuth & Permissions: if it differs from what's in `.env`, copy the new value over `SLACK_BOT_TOKEN` in `.env` and tell me to re-push secrets. Tell me when verified + reinstalled + token confirmed." | After operator confirms reinstall: `grep '^SLACK_BOT_TOKEN=xoxb-' .env | cut -c1-25` (prints prefix only, never the full token); ask operator if the prefix in `.env` matches the prefix shown at the top of OAuth & Permissions in Slack. If they say no: re-run `supabase secrets set --env-file .env --project-ref $REF` after they update `.env`. On verification failure, open the Supabase Dashboard at Project → Edge Functions → `ingest-thought` → **Logs** and parse output. Diagnose via `references/error-recovery.md` Slack Step 8 section. | Paste URL, verify, add event, save, manually reinstall, compare token prefix. |
| **9. Smoke test each channel** | "Let's test. In `#brain-capture`, post: 'First Slack capture test from setup wizard.' The bot should reply within 5 seconds in-thread with an extracted summary." Then offer optional tests: thread reply, query prefix, silent brain channel, digest delivery. Operator can skip any. | For digest delivery: `source .env && curl -X POST "$SUPABASE_URL/functions/v1/brain-digest?mode=daily" -H "x-brain-key: $MCP_ACCESS_KEY"`. Header auth keeps the key out of Edge Function request logs (the legacy `?key=$MCP_ACCESS_KEY` URL form would also work but logs the key plaintext). Check response for `"status":"delivered"` or `"status":"skipped"` (both OK). If you see `"status":"delivery_failed"` with a `slack_error` field, look up the Slack error code (`invalid_auth` = bot token regenerated on a recent reinstall, `not_in_channel` = bot not invited to `SLACK_DIGEST_CHANNEL`). | Post test messages, confirm bot replies. |
...
### Step 8: manual reinstall after Save Changes is mandatory, not optional

Slack's "reinstall required" prompt after Save Changes only fires for some scope and event combinations and is silent for others. Operators who follow the doc literally and only reinstall when prompted end up with green Verified, all green config, and zero inbound message events delivered to the function. Always do the manual reinstall via OAuth & Permissions → Reinstall to Workspace, every time, even when no orange dot appears. The cost is a five-second click; the cost of skipping it is a silent-fail debugging session.

### Step 8: bot tokens can regenerate during reinstall

Slack does not always rotate the `xoxb-` Bot User OAuth Token on reinstall, but it sometimes does (especially after scope changes). A regenerated token leaves the `.env` value stale. Outbound calls (digest delivery) then fail with `slack_error: invalid_auth`; inbound capture fails silently because the function rejects events as if from a stranger. The skill emits a token-prefix comparison after the reinstall step (without printing the full token to chat) so the operator can confirm in real time whether `.env` needs updating before moving to Step 9.
...
- **"URL did not respond with the value of the challenge parameter"** plus `HMAC verification failed` in the `ingest-thought` Logs (Supabase Dashboard → Edge Functions → `ingest-thought` → **Logs**): `SLACK_SIGNING_SECRET` in Supabase doesn't match what this Slack app is signing with. Double-check you copied from the right app's Basic Information page (easy to paste from an older app by accident), update `.env`, re-run `supabase secrets set --env-file .env --project-ref $REF`, retry verification.
...
- **Green "Verified" but Step 9 smoke test sees no reply and no row in `thoughts`**: most likely the manual reinstall in Step 8 was skipped (the prompt is unreliable; manual reinstall is mandatory) or the bot token regenerated during reinstall and `.env` is stale. Check the Bot User OAuth Token at the top of OAuth & Permissions and compare its prefix against `.env`. If different, update `.env` and re-push secrets. Less likely: Event Subscriptions saved without `message.channels` under Subscribe to bot events.
