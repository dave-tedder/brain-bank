...
MAIN-CHECKOUT ANCHOR: write deliverables to the MAIN checkout, never into a git worktree copy. Some runtimes run a session in a worktree without telling it, and `git rev-parse --git-common-dir` versus `--git-dir` tells you which you are in — they differ inside a worktree, and the common dir points at the main checkout's `.git`. A worktree is a SEPARATE working directory: the closeout controller, both critic lanes, and the sweep all read the MAIN checkout only. A deliverable written to a worktree's `deliverables/` is physically real, is reported honestly in the receipt, and is unreachable by every downstream reader — and it dies with the worktree. Resolve the main checkout once, write there by absolute path, and run the durability push from there too: the push script anchors to its own location, so invoking the worktree's copy of it stages the wrong tree and reports `NOTHING_TO_COMMIT` while the real file sits unpushed in main.

VERIFY BEFORE YOU NAME IT: after writing, read the file back from the main checkout by absolute path, then append ` @ MAIN-VERIFIED` to the path under "Touched files or records:". If that read fails, append ` @ NOT-VERIFIED-FROM-MAIN (<what failed>)` and name it under "Limitations:". Never list a `deliverables/` path without exactly one of those two tokens — "I just wrote it" is not verification, it is precisely the check that passes inside a worktree while the file is stranded. Token order when both apply: `<path> @ MAIN-VERIFIED @ UNPUSHED (<reason>)`.
