...
This applies every SQL file in `supabase/migrations/` to your Supabase project in order. There are eleven migrations: the `extensions` migration (pgvector, pgcrypto, pg_cron, pg_net), the core tables (`thoughts`, `action_items`, `clients`, `client_sessions`, `content_items`, `business_events`, `compiled_pages`, `notion_mappings`, `digests`), and the `match_thoughts` similarity RPC.

**What success looks like:** `Applying migration 0000_extensions.sql...` through `Applying migration 0010_match_thoughts_rpc.sql...`, each followed by `Finished supabase db push.`
...
Verify the schema landed. In the Supabase dashboard, go to Database → Tables. You should see ten tables: `thoughts`, `action_items`, `clients`, `client_sessions`, `content_items`, `business_events`, `compiled_pages`, `notion_mappings`, `digests`, and an internal `schema_migrations`.
...
**What success looks like:** JSON response like `{"content":[{"type":"text","text":"Captured. ..."}]}`, returned in under five seconds.
