...
- **Middleware MUST live at `src/middleware.ts`, not project root.** Next.js silently ignores a root-level `middleware.ts` when the app directory is at `src/app/` — no warning, no error, no `next build` failure, just the entire site served to anonymous traffic. The dashboard shipped with this exact bug from initial commit `42bfd3d` until PR #2 (`833d859`, 2026-04-17). Detection: `npm run build` output must include a `ƒ Middleware  NN kB` line. If absent, middleware isn't running. See `~/.claude/rules/nextjs-middleware-auth.md` for the full auth-trap trilogy.
- **Auth check must explicitly handle unset env var.** `cookie !== process.env.DASHBOARD_PASSWORD` fails open when the env var is missing (`undefined !== undefined` is `false`). Use `!expected || !auth || auth !== expected` instead. Hardened in PR #3.
- **Route handler redirects must use relative `Location` headers.** On Railway standalone, `request.url` inside Route Handlers dereferences the internal bind (`http://localhost:8080`). `NextResponse.redirect(new URL("/login", request.url))` leaks `localhost:8080` into production `Location` headers. Fixed in PR #5. Middleware's `NextRequest.url` is fine — the same syntactic call only breaks in route handlers.
