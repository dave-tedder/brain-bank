...
-- Update check constraint to allow antigravity as a valid runtime alongside claude, codex, and triage-critic
ALTER TABLE public.agent_task_ledger DROP CONSTRAINT IF EXISTS agent_task_ledger_runtime_check;
ALTER TABLE public.agent_task_ledger ADD CONSTRAINT agent_task_ledger_runtime_check CHECK (runtime IN ('claude', 'codex', 'triage-critic', 'antigravity'));
