-- OE-1 review follow-up: align manual receipt/status handling with the
-- canonical Open Engine state-rule table before OE-2/OE-3 build on it.
--
-- The first OE-1 helper was intentionally narrow for the manual board, but
-- OE-3 MCP tools need the DB to accept unchanged AGENT FOLLOW-UP, AGENT STATUS,
-- and skill receipts on non-done tasks. High-risk tasks still cannot enter
-- Agent Working without explicit approval.
