-- OE-5 review follow-up: remove the legacy one-argument claim helper.
-- The scheduled runner must pass an explicit max-risk cap, and old callers
-- should fail loudly instead of silently claiming medium-risk work.

drop function if exists public.claim_next_agent_task(text);
drop function if exists public.claim_next_agent_task(text, text);

create function public.claim_next_agent_task(
  p_agent_code text,
  p_max_risk text
)
