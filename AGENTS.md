...
- **Check live state before describing it.** Any receipt, operator step, briefing line, or report that asserts the CURRENT state of a live surface — a post's status, whether a page exists or 404s, what a published page actually says, whether a listing is claimed — must fetch that surface in the same run and report what was seen, with the date. Written artifacts describing live state (a packet's `context`, a prior deliverable, a spec, an older `operator_action` note) were accurate when written; reality moves and nothing rewrites them. On a live board, one card's operator step said a page was "currently a draft, hit Publish" while the page was in fact scheduled to publish itself a week later — acting on that note would have published early and collapsed deliberate content spacing. One read of the live page caught it, and four separate instances of the same shape surfaced in a single week. When the surface cannot be reached, say so under `Limitations:` and write the step so it does not assert a state nobody checked; "the packet says" is not a check. Deliberately PROSE, not enforced: the enforced version (a closeout hold when a receipt names a live surface with no evidence of a fetch) is a bigger build. It is one rule, not a source-of-truth precedence document, which was considered and narrowed away because a precedence declaration would have caught none of the real instances.
